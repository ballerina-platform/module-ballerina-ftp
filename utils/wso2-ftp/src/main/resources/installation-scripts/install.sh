...
then
    if [ ! -e temp ]
    then mkdir temp
    cp $ballerina_lib_location/wso2-$package_name-module-$version.jar temp/
    fi
fi

cp dependencies/wso2-$package_name-module-$version.jar $ballerina_lib_location
...
if [ $? -ne 0 ]; then
    ...
    if [ -e temp/wso2-$package_name-module-$version.jar ]
    then cp temp/wso2-$package_name-module-$version.jar $ballerina_lib_location/
    rm -r temp
    else rm $ballerina_lib_location/wso2-$package_name-module-$version.jar
    fi
    echo "You can install the module by manually copying"
    echo 1. "dependencies/wso2-$package_name-module-$version.jar to $ballerina_lib_location"
    echo 2. "Contents of balo directory to $ballerina_balo_location"
