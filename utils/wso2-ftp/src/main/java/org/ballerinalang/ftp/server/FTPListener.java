...
public class FTPListener implements RemoteFileSystemListener {
    ...
    private MapValue<String, Object> getSignatureParameters(RemoteFileSystemEvent fileSystemEvent) {
        ...
        MapValue<String, Object> watchEventStruct = BallerinaValues.createRecordValue(
                new BPackage("wso2", "ftp"), FtpConstants.FTP_SERVER_EVENT);

        // For newly added files
        ArrayValue addedFiles = new ArrayValue(getFileInfoType());
        int i = 0;
        for (FileInfo info : fileSystemEvent.getAddedFiles()) {
            ...
            final MapValue<String, Object> fileInfo = BallerinaValues.createRecordValue(
                    new BPackage("wso2", "ftp", "0.1.0"), FtpConstants.FTP_FILE_INFO, fileInfoParams);
...
//      For deleted files
    ...
    private BType getFileInfoType() {
        MapValue<String, Object> fileInfoStruct = BallerinaValues.createRecordValue(
                new BPackage("wso2", "ftp"), FtpConstants.FTP_FILE_INFO);
