...
public function testClientWithInvalidFailureThresholdHigh() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail with invalid failure threshold > 1.0");
    if cbClient is ftp:Error {
...
public function testClientWithInvalidFailureThresholdLow() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail with invalid failure threshold < 0.0");
    if cbClient is ftp:Error {
        test:assertTrue(cbClient.message().includes("failureThreshold"),
            msg = "Error message should mention failureThreshold");
    }
...
public function testClientWithInvalidBucketSize() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail when bucketSize > timeWindow");
    if cbClient is ftp:Error {
...
public function testClientWithInvalidRequestVolumeThreshold() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail when requestVolumeThreshold <= 0");
    if cbClient is ftp:Error {
...
public function testClientWithInvalidResetTime() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail when resetTime <= 0");
    if cbClient is ftp:Error {
...
public function testClientWithEmptyFailureCategories() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail when failureCategories is empty");
    if cbClient is ftp:Error {
...
public function testClientWithNonDivisibleTimeWindow() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail when timeWindow is not divisible by bucketSize");
    if cbClient is ftp:Error {
        test:assertTrue(cbClient.message().includes("timeWindow") || cbClient.message().includes("bucketSize"),
            msg = "Error message should mention timeWindow or bucketSize");
    }
...
public function testOperationsWithCircuitBreaker() returns error? {
    ...
    ftp:Client cbClient = check new (config);
    ...
    ftp:FileInfo[] files = check cbClient->list("/home/in");
...
public function testClientCreationFailsForNonExistentServer() {
    ...
    ftp:Client|ftp:Error cbClient = new (config);
    test:assertTrue(cbClient is ftp:Error, msg = "Client creation should fail for non-existent server");
}
...
public function testCircuitBreakerOpensAfterFailures() returns error? {
    ...
    // First failure
    byte[]|ftp:Error result1 = cbClient->getBytes(nonExistentPath);
    test:assertTrue(result1 is ftp:Error, msg = "First request should fail (file not found)");
    test:assertFalse(result1 is ftp:CircuitBreakerOpenError,
    ...
    byte[]|ftp:Error result2 = cbClient->getBytes(nonExistentPath);
    test:assertTrue(result2 is ftp:Error, msg = "Second request should fail");
    test:assertFalse(result2 is ftp:CircuitBreakerOpenError,
    ...
    byte[]|ftp:Error result3 = cbClient->getBytes(nonExistentPath);
    test:assertTrue(result3 is ftp:CircuitBreakerOpenError,
        msg = "Third request should fail with CircuitBreakerOpenError (circuit is open)");

    if result3 is ftp:CircuitBreakerOpenError {
...
public function testCircuitBreakerHalfOpenTrial() returns error? {
    ...
    byte[]|ftp:Error result1 = cbClient->getBytes(nonExistentPath);
    test:assertTrue(result1 is ftp:Error, msg = "Initial request should fail");

    // Circuit should now be OPEN
    byte[]|ftp:Error result2 = cbClient->getBytes(nonExistentPath);
    test:assertTrue(result2 is ftp:CircuitBreakerOpenError,
    ...
    byte[]|ftp:Error trialResult = cbClient->getBytes(nonExistentPath);
    test:assertFalse(trialResult is ftp:CircuitBreakerOpenError,
...
public function testCircuitBreakerHalfOpenSuccessfulTrial() returns error? {
    ...
    boolean|ftp:Error trialResult = cbClient->exists("/home/in");
    test:assertFalse(trialResult is ftp:CircuitBreakerOpenError,
        msg = "HALF_OPEN should allow a single trial request");
    ...
    boolean|ftp:Error result3 = cbClient->exists("/home/in");
    test:assertFalse(result3 is ftp:CircuitBreakerOpenError,
...
public function testRetryAndCircuitBreakerInteraction() returns error? {
    ...
    byte[]|ftp:Error result1 = cbClient->getBytes(nonExistentPath);
    test:assertTrue(result1 is ftp:AllRetryAttemptsFailedError,
    ...
    byte[]|ftp:Error result2 = cbClient->getBytes(nonExistentPath);
    test:assertTrue(result2 is ftp:AllRetryAttemptsFailedError,
