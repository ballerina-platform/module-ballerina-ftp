...
ftp:ListenerConfiguration anonymousRemoteServerConfig = {
    protocol: ftp:FTP,
    host: "127.0.0.1",
    auth: {
        credentials: {
...
ftp:Service anonymousRemoteServerService = service object {
...
public function testAnonServerAddedFile() {
    ...
    while timeoutInSeconds > 0 {
        if anonServerAddedFileInfo is ftp:FileInfo {
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).path,
                "ftp://anonymous:anything@127.0.0.1:21210/home/in/test1.txt");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).size, 12);
            test:assertTrue((<ftp:FileInfo>anonServerAddedFileInfo).lastModifiedTimestamp > 0);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).name, "test1.txt");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isFolder, false);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isFile, true);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).pathDecoded, "/home/in/test1.txt");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).extension, "txt");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).publicURIString,
                "ftp://anonymous:***@127.0.0.1:21210/home/in/test1.txt");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).fileType, "file");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isAttached, true);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isContentOpen, false);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isExecutable, false);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isHidden, false);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isReadable, true);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).isWritable, true);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).depth, 4);
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).scheme, "ftp");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).uri,
                "//anonymous:anything@127.0.0.1:21210/home/in/test1.txt");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).rootURI,
                "ftp://anonymous:anything@127.0.0.1:21210/");
            test:assertEquals((<ftp:FileInfo>anonServerAddedFileInfo).friendlyURI,
                "ftp://anonymous:***@127.0.0.1:21210/home/in/test1.txt");
            break;
        } else {
            runtime:sleep(1);
...
public function testFtpServerDeregistration() returns error? {
    ftp:Listener detachFtpServer = check new ({
        protocol: ftp:FTP,
        host: "127.0.0.1",
        auth: {
            credentials: {
    ...
    ftp:Service detachService = service object {
        remote function onFileChange(ftp:WatchEvent & readonly event) {
        }
    };
...
public function testServerRegisterFailureEmptyPassword() returns error? {
    ftp:Listener emptyPasswordServer = check new ({
        protocol: ftp:FTP,
        host: "127.0.0.1",
        auth: {
            credentials: {
    ...
    ftp:Service attachService = service object {
        remote function onFileChange(ftp:WatchEvent & readonly event) {
        }
    };
    error? attachResult = emptyPasswordServer.attach(attachService);
    if attachResult is ftp:Error {
...
public function testServerRegisterFailureEmptyUsername() returns error? {
    ftp:Listener|ftp:Error emptyUsernameServer = new ({
        protocol: ftp:FTP,
        host: "127.0.0.1",
        auth: {
            credentials: {
    ...
    if emptyUsernameServer is ftp:Error {
...
public function testServerRegisterFailureInvalidUsername() returns error? {
    ftp:Listener invalidUsernameServer = check new (
        protocol = ftp:FTP,
        host = "127.0.0.1",
        auth = {
            credentials: {
...
public function testServerRegisterFailureInvalidPassword() returns error? {
    ftp:Listener invalidPasswordServer = check new ({
        protocol: ftp:FTP,
        host: "127.0.0.1",
        auth: {
            credentials: {
...
public function testConnectToInvalidUrl() returns error? {
    ftp:Listener invalidUrlServer = check new ({
        protocol: ftp:FTP,
        host: "localhost",
        port: 21218,
        auth: {
...
public function testServerRegisterFailureInvalidCredentialsWithDetails() returns error? {
    ftp:Listener invalidCredsServer = check new ({
        protocol: ftp:FTP,
        host: "127.0.0.1",
        auth: {
            credentials: {
...
public function testMutableWatchEvent() returns error? {
    ftp:Service ftpService = service object {
        remote function onFileChange(ftp:WatchEvent event) {
            event.addedFiles.forEach(function (ftp:FileInfo fInfo) {
                fInfo.name = "overriden_name";
            });
            fileInfos = event.addedFiles;
        }
    };
    ftp:Listener ftpListener = check new ({
        protocol: ftp:FTP,
        host: "localhost",
        port: 21212,
        auth: {
    ...
    check (<ftp:Client>triggerClient)->put("/home/in/mutable/test1.txt", bStream, compressionType = ftp:ZIP);
    runtime:sleep(2);
    ...
    fileInfos.forEach(function (ftp:FileInfo fInfo) {
...
public function testValidateDeletedFilesFromListener() returns error? {
    ...
    check (<ftp:Client>triggerClient)->put("/home/in/deleteFile1.txt", bStream1);
    stream<io:Block, io:Error?> bStream2 = check io:fileReadBlocksAsStream(putFilePath, 5);
    check (<ftp:Client>triggerClient)->put("/home/in/deleteFile2.txt", bStream2);
    stream<io:Block, io:Error?> bStream3 = check io:fileReadBlocksAsStream(putFilePath, 5);
    check (<ftp:Client>triggerClient)->put("/home/in/deleteFile3.txt", bStream3);

    runtime:sleep(2);
    check (<ftp:Client>triggerClient)->delete("/home/in/deleteFile1.txt");
    check (<ftp:Client>triggerClient)->delete("/home/in/deleteFile2.txt");
    check (<ftp:Client>triggerClient)->delete("/home/in/deleteFile3.txt");
    runtime:sleep(5);
...
public function testIsolatedService() returns error? {
    ...
    stream<io:Block, io:Error?> bStream1 = check io:fileReadBlocksAsStream(putFilePath, 5);
    check (<ftp:Client>triggerClient)->put("/home/in/isolatedTestFile.isolated", bStream1);
    runtime:sleep(5);

    runtime:deregisterListener(ftpListener);
    check ftpListener.gracefulStop();
    check (<ftp:Client>triggerClient)->delete("/home/in/isolatedTestFile.isolated");
    lock {
...
public function testImmediateStop() returns error? {
    ...
    stream<io:Block, io:Error?> bStream = check io:fileReadBlocksAsStream(putFilePath, 5);
    check (<ftp:Client>triggerClient)->put(remotePath, bStream);
    runtime:sleep(3);
    ...
    check (<ftp:Client>triggerClient)->delete(remotePath);
}
