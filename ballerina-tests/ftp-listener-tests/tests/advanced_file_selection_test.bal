...
ftp:Client ftpClient = check new ({protocol: ftp:FTP, host: "127.0.0.1", port: 21212, auth: {credentials: {username: "wso2", password: "wso2123"}}, userDirIsRoot: false});
...
function testFileAgeFilterRespectsMaxAge() returns error? {
    ...
    ftp:Service ageService = service object {
...
function testFileDependencyFiltering() returns error? {
    ...
    ftp:Service dependencyService = service object {
        remote function onFileChange(ftp:WatchEvent & readonly event) {
            if event.addedFiles.length() == 0 {
                return;
            }
    ...
    ftp:Listener dependencyListener = check new ({
        protocol: ftp:FTP,
        host: "127.0.0.1",
        port: 21212,
        auth: {
...
function ensureDirectory(ftp:Client ftpClient, string path) returns error? {
...
function removeIfExists(ftp:Client ftpClient, string path) returns error? {
