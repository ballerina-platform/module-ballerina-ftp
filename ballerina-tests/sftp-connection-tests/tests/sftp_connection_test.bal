...
// Disabled: this test intermittently hangs in CI against the Apache Mina
// SSHD mock server — the SFTP listener poller does not always terminate
// cleanly, and even the deregister-only teardown pattern has not proven
// reliable. Kept in the suite so it can be re-enabled once the underlying
// shutdown path in the listener is investigated further.
@test:Config {
    enable: false,
...
function testSftpListener_DetectsNewFile() returns error? {
    ...
    // Run cleanup first — deregister the listener and close the client — so
    // that even if the assertion below fails, the poller and client do not
    // leak into subsequent tests. gracefulStop() is skipped because it can
    // block indefinitely on SFTP/SSH teardown in the test environment.
