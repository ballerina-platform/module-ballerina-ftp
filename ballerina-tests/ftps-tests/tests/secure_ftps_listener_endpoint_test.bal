...
function removeFtpsFileIfExists(ftp:Client ftpClient, string path) returns error? {
...
ftp:ClientConfiguration triggerClientConfig = {
    protocol: ftp:FTPS,
    host: "127.0.0.1",
    port: 21214,
...
ftp:ClientConfiguration triggerImplicitClientConfig = {
    protocol: ftp:FTPS,
    host: "127.0.0.1",
    port: 21217,
    connectTimeout: 60.0, // Increase to 60s for GraalVM/CI stability
...
function testFtpsExplicitListener() returns error? {
    ...
    ftp:Client triggerClient = check new(triggerClientConfig);
    check removeFtpsFileIfExists(triggerClient, targetFile);
    resetFtpsState();

    ftp:Service ftpsService = service object {
...
function testFtpsImplicitListener() returns error? {
    ...
    ftp:Client triggerClient = check new(triggerImplicitClientConfig);
    check removeFtpsFileIfExists(triggerClient, targetFile);
    resetFtpsState();
...
public function testFtpsConnectWithInvalidKeystore() returns error? {
    ftp:Listener ftpsServer = check new ({
        protocol: ftp:FTPS,
        host: "localhost",
        port: 21214,
        auth: {
...
public function testFtpsConnectToFTPServerWithFTPProtocol() returns error? {
    ftp:Listener|ftp:Error ftpsServer = new ({
        protocol: ftp:FTP,
        host: "localhost",
        port: 21214,
        auth: {
