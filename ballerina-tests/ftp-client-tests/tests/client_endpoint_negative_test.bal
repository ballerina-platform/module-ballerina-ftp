...
public function testConnectionWithNonExistingServer() returns error? {
    ftp:ClientConfiguration nonExistingServerConfig = {
        protocol: ftp:FTP,
        host: "127.0.0.1",
        port: 21218,
        auth: {credentials: {username: "wso2", password: "wso2123"}}
    };
    ftp:Client|ftp:Error nonExistingServerClientEp = new (nonExistingServerConfig);
    test:assertTrue(nonExistingServerClientEp is ftp:ConnectionError,
        msg = "Expected ConnectionError when connecting to non-existing server");
    if nonExistingServerClientEp is ftp:ConnectionError {
...
public function testConnectionWithInvalidConfiguration() returns error? {
    ...
    ftp:Client|ftp:Error invalidServerClientEp = new (invalidConfig);
    if invalidServerClientEp is ftp:Error {
...
public function testReadNonExistingFile() returns error? {
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get("/home/in/nonexisting.txt");
    test:assertTrue(str is ftp:FileNotFoundError,
        msg = "Expected FileNotFoundError when reading non-existing file");
    if str is ftp:FileNotFoundError {
...
public function testAppendContentToNonExistingFile() returns error? {
    ...
    ftp:Error? receivedError = (<ftp:Client>clientEp)->append("/../invalidFile", bStream);
    if receivedError is ftp:Error {
...
public function testPutFileContentAtInvalidFileLocation() returns error? {
    ...
    ftp:Error? receivedError = (<ftp:Client>clientEp)->put("/../InvalidFile", bStream);
    if receivedError is ftp:Error {
        test:assertEquals(receivedError.message(), "Invalid relative file name.",
...
public function testIsDirectoryWithNonExistingDirectory() {
    boolean|ftp:Error receivedError = (<ftp:Client>clientEp)->isDirectory("/home/in/nonexisting");
    test:assertTrue(receivedError is ftp:FileNotFoundError,
        msg = "Expected FileNotFoundError when checking isDirectory on non-existing path");
    if receivedError is ftp:FileNotFoundError {
...
public function testCreateDirectoryAtInvalidLocation() {
    ftp:Error? receivedError = (<ftp:Client>clientEp)->mkdir("/../InvalidDirectory");
    if receivedError is ftp:Error {
        test:assertEquals(receivedError.message(), "Invalid relative file name.",
...
public function testRenameNonExistingDirectory() {
    ...
    ftp:Error? receivedError = (<ftp:Client>clientEp)->rename(existingName, newName);
...
public function testGetFileSizeFromNonExistingFile() {
    int|ftp:Error receivedError = (<ftp:Client>clientEp)->size("/nonExistingFile");
    if receivedError is ftp:Error {
...
public function testListFilesFromNonExistingDirectory() {
    ftp:FileInfo[]|ftp:Error receivedError = (<ftp:Client>clientEp)->list("/nonExistingDirectory");
    if receivedError is ftp:Error {
...
public function testDeleteFileAtNonExistingLocation() returns error? {
    ftp:Error? receivedError = (<ftp:Client>clientEp)->delete("/nonExistingFile");
    test:assertTrue(receivedError is ftp:FileNotFoundError,
...
public function testRemoveDirectoryWithWrongUrl() {
    ftp:Error? receivedError = (<ftp:Client>clientEp)->rmdir("/nonExistingDirectory");
    test:assertTrue(receivedError is ftp:FileNotFoundError,
...
public function testSFTPConnectionToFTPServer() returns error? {
    ftp:ClientConfiguration serverConfig = {
        protocol: ftp:SFTP,
        host: "127.0.0.1",
        port: 21212,
        auth: {
    ...
    ftp:Client|ftp:Error clientEp = new (serverConfig);
    if clientEp is ftp:Error {
...
public function testConnectionWithInvalidHostDetailedError() returns error? {
    ftp:ClientConfiguration invalidHostConfig = {
        protocol: ftp:FTP,
        host: "invalid.nonexistent.host.example",
        port: 21,
        auth: {credentials: {username: "wso2", password: "wso2123"}}
    };
    ftp:Client|ftp:Error invalidHostClientEp = new (invalidHostConfig);
    test:assertTrue(invalidHostClientEp is ftp:ConnectionError,
        msg = "Expected ConnectionError when connecting to invalid host");
    if invalidHostClientEp is ftp:ConnectionError {
...
public function testMoveNonExistingFile() returns error? {
    ...
    ftp:Error? result = (<ftp:Client>clientEp)->move(nonExistingPath, destinationPath);
    test:assertTrue(result is ftp:FileNotFoundError,
        msg = "Expected FileNotFoundError when moving non-existing file");
    if result is ftp:FileNotFoundError {
...
public function testCopyNonExistingFile() returns error? {
    ...
    ftp:Error? result = (<ftp:Client>clientEp)->copy(nonExistingPath, destinationPath);
    test:assertTrue(result is ftp:FileNotFoundError,
...
public function testCopyToExistingFile() returns error? {
    ...
    ftp:Error? cleanupResult1 = (<ftp:Client>clientEp)->delete(sourcePath);
    ftp:Error? cleanupResult2 = (<ftp:Client>clientEp)->delete(destinationPath);

    // Create source file
    stream<io:Block, io:Error?> sourceStream = check io:fileReadBlocksAsStream(putFilePath, 5);
    check (<ftp:Client>clientEp)->put(sourcePath, sourceStream);

    // Create destination file
    stream<io:Block, io:Error?> destStream = check io:fileReadBlocksAsStream(putFilePath, 5);
    check (<ftp:Client>clientEp)->put(destinationPath, destStream);

    // Copy to existing file should fail with FileAlreadyExistsError
    ftp:Error? result = (<ftp:Client>clientEp)->copy(sourcePath, destinationPath);
    test:assertTrue(result is ftp:FileAlreadyExistsError,
        msg = "Expected FileAlreadyExistsError when copying to existing file");
    if result is ftp:FileAlreadyExistsError {
        test:assertTrue(result.message().includes("already exists"),
            msg = "Error message should indicate file already exists");
    }

    // Cleanup
    ftp:Error? cleanup1 = (<ftp:Client>clientEp)->delete(sourcePath);
    ftp:Error? cleanup2 = (<ftp:Client>clientEp)->delete(destinationPath);
}
...
public function testExistsNonExistingFile() returns error? {
    ...
    boolean|ftp:Error result = (<ftp:Client>clientEp)->exists(nonExistingPath);
...
public function testListenerWithInvalidFileNamePattern() returns error? {
    ftp:ListenerConfiguration invalidPatternConfig = {
        protocol: ftp:FTP,
        host: "127.0.0.1",
        port: 21212,
        auth: {credentials: {username: "wso2", password: "wso2123"}},
    ...
    ftp:Listener|ftp:Error listenerResult = new (invalidPatternConfig);
    test:assertTrue(listenerResult is ftp:InvalidConfigError,
        msg = "Expected InvalidConfigError when creating listener with invalid fileNamePattern");
    if listenerResult is ftp:InvalidConfigError {
...
public function testListenerWithInvalidDependencyTargetPattern() returns error? {
    ftp:ListenerConfiguration invalidDependencyConfig = {
        protocol: ftp:FTP,
        host: "127.0.0.1",
        port: 21212,
        auth: {credentials: {username: "wso2", password: "wso2123"}},
    ...
    ftp:Listener listenerResult = check new (invalidDependencyConfig);
    ...
    test:assertTrue(attachResult is ftp:InvalidConfigError,
        msg = "Expected InvalidConfigError for invalid dependency targetPattern");
    if attachResult is ftp:InvalidConfigError {
...
public function testListenerWithInvalidDependencyRequiredFilePattern() returns error? {
    ftp:ListenerConfiguration invalidRequiredFileConfig = {
        protocol: ftp:FTP,
        host: "127.0.0.1",
        port: 21212,
        auth: {credentials: {username: "wso2", password: "wso2123"}},
    ...
    ftp:Listener listenerResult = check new (invalidRequiredFileConfig);
    ftp:Service attachService = service object {
        remote function onFileChange(ftp:WatchEvent & readonly event) {
        }
    };
