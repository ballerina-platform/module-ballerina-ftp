...
public function testFileAlreadyExistsErrorOnMkdir() returns error? {
    ...
    ftp:Error? createResult = (<ftp:Client>clientEp)->mkdir(dirPath);

    // Try to create it again - should fail with FileAlreadyExistsError
    ftp:Error? result = (<ftp:Client>clientEp)->mkdir(dirPath);
    ...
    // Cleanup
    ftp:Error? cleanup = (<ftp:Client>clientEp)->rmdir(dirPath);
}
...
public function testFileAlreadyExistsErrorOnRename() returns error? {
    ...
    // Cleanup
    ftp:Error? cleanupSrc = (<ftp:Client>clientEp)->delete(sourcePath);
    ftp:Error? cleanupDest = (<ftp:Client>clientEp)->delete(destPath);
}
...
public function testFileNotFoundErrorOnGet() returns error? {
    stream<byte[] & readonly, io:Error?>|ftp:Error result = (<ftp:Client>clientEp)->get("/home/in/nonexistent_error_test.txt");
...
public function testFileNotFoundErrorOnDelete() returns error? {
    ftp:Error? result = (<ftp:Client>clientEp)->delete("/home/in/nonexistent_delete_test.txt");
    test:assertTrue(result is ftp:FileNotFoundError,
...
public function testInvalidConfigErrorOnBadRegex() {
    ...
    ftp:Listener|ftp:Error result = new (config);
