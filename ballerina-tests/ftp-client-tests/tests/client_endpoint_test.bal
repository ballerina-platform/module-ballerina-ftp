...
ftp:ClientConfiguration anonConfig = {
    protocol: ftp:FTP,
    host: "127.0.0.1",
    port: 21210
};
...
// Create the config to access mock FTP server
ftp:ClientConfiguration configLaxDataBinding = {
    protocol: ftp:FTP,
    host: "127.0.0.1",
    port: 21212,
    auth: {credentials: {username: "wso2", password: "wso2123"}},
...
ftp:ClientConfiguration ftpUserHomeRootConfig = {
    protocol: ftp:FTP,
    host: "127.0.0.1",
    port: 21212,
    auth: {credentials: {username: "wso2", password: "wso2123"}},
...
ftp:Client? anonClientEp = ();
ftp:Client? clientEp = ();
ftp:Client? clientEpLaxDataBinding = ();
ftp:Client? sftpClientEp = ();
ftp:Client? sftpClientUserDirRootEp = ();
ftp:Client? ftpUserHomeRootClientEp = ();
...
public function testReadFromAnonServer() returns error? {
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>anonClientEp)->get(filePath);
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, "File content"), msg = "Found unexpected content from `get` operation");
...
@test:Config {
    dependsOn: [testReadFromAnonServer]
}
public function testReadBlockFittingContent() returns error? {
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(filePath);
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, "File content"), msg = "Found unexpected content from `get` operation");
        io:Error? closeResult = str.close();
        if closeResult is io:Error {
...
public function testReadBlockNonFittingContent() returns error? {
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(nonFittingFilePath);
    int i = 0;
    ...
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, nonFittingContent), msg = "Found unexpected content from `get` operation");
        io:Error? closeResult = str.close();
        if closeResult is io:Error {
            test:assertFail(msg = "Error while closing stream in `get` operation." + closeResult.message());
        }
...
public function testAppendContent() returns error? {
    ...
    ftp:Error? response = (<ftp:Client>clientEp)->append(filePath, bStream);
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(filePath);
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, "File contentAppend content"),
...
public function testPutFileContent() returns error? {
    ...
    ftp:Error? response = (<ftp:Client>clientEp)->put(newFilePath, bStream);
    if response is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(newFilePath);
    if str is stream<byte[] & readonly, io:Error?> {
...
public function testPutCompressedFileContent() returns error? {
    ...
    ftp:Error? response = (<ftp:Client>clientEp)->put("/home/in/test3.txt", bStream, compressionType = ftp:ZIP);
    if response is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get("/home/in/test3.zip");
    if str is ftp:Error {
...
function testPutBytes() returns error? {
    ...
    check (<ftp:Client>clientEp)->putBytes(path, content);
    byte[] got = check (<ftp:Client>clientEp)->getBytes(path);
...
function testGetBytesAsStream() returns error? {
    ...
    check (<ftp:Client>clientEp)->putBytes(path, content);
    stream<byte[], error?> got = check (<ftp:Client>clientEp)->getBytesAsStream(path);
...
function testPutJson() returns error? {
    ...
    check (<ftp:Client>clientEp)->putJson(path, j);
    json got = check (<ftp:Client>clientEp)->getJson(path);
...
function testPutXml() returns error? {
    ...
    check (<ftp:Client>clientEp)->putXml(path, x);
    xml got = check (<ftp:Client>clientEp)->getXml(path);
...
function testPutXmlFailure() returns error? {
    ...
    check (<ftp:Client>clientEp)->putText(path, txt);
    xml|ftp:Error got = (<ftp:Client>clientEp)->getXml(path);

    // Compare string representations for stability
    test:assertTrue(got is ftp:Error, msg = "XML content binding should have failed for non-XML content");
}
...
function testGetJsonContentBindingError() returns error? {
    ...
    check (<ftp:Client>clientEp)->putText(path, "this is not valid json {{{");

    json|ftp:Error result = (<ftp:Client>clientEp)->getJson(path);
    test:assertTrue(result is ftp:ContentBindingError, msg = "getJson should return ContentBindingError for invalid JSON");
    ...
    // Cleanup
    check (<ftp:Client>clientEp)->delete(path);
}
...
function testGetXmlContentBindingError() returns error? {
    ...
    check (<ftp:Client>clientEp)->putText(path, "this is not valid xml <<<>>>");

    xml|ftp:Error result = (<ftp:Client>clientEp)->getXml(path);
    test:assertTrue(result is ftp:ContentBindingError, msg = "getXml should return ContentBindingError for invalid XML");
    if result is ftp:ContentBindingError {
        test:assertTrue(result.detail().filePath is string, msg = "ContentBindingError should contain filePath");
        test:assertTrue(result.detail().content is byte[], msg = "ContentBindingError should contain content bytes");
    }
...
function testGetJsonTypedContentBindingError() returns error? {
    ...
    check (<ftp:Client>clientEp)->putJson(path, content);

    // Try to bind to a record that requires 'age' field
    PersonStrict|ftp:Error result = (<ftp:Client>clientEp)->getJson(path);
    test:assertTrue(result is ftp:ContentBindingError, msg = "getJson should return ContentBindingError when type binding fails");
    if result is ftp:ContentBindingError {
        test:assertTrue(result.detail().filePath is string, msg = "ContentBindingError should contain filePath");
        test:assertTrue(result.detail().content is byte[], msg = "ContentBindingError should contain content bytes");
    }
...
function testGetCsvContentBindingError() returns error? {
    ...
    check (<ftp:Client>clientEp)->putCsv(path, csvData);

    // Try to bind to a record that expects 'age' as int
    CsvPersonStrict[]|ftp:Error result = (<ftp:Client>clientEp)->getCsv(path);
    test:assertTrue(result is ftp:ContentBindingError, msg = "getCsv should return ContentBindingError when type binding fails");
    if result is ftp:ContentBindingError {
        test:assertTrue(result.detail().filePath is string, msg = "ContentBindingError should contain filePath");
        test:assertTrue(result.detail().content is byte[], msg = "ContentBindingError should contain content bytes");
    }
...
function testGetCsvAsStreamContentBindingError() returns error? {
    ...
    check (<ftp:Client>clientEp)->putText(path, "name,age\nAlice,\\q\nBob,25");

    stream<string[], error?> result = check (<ftp:Client>clientEp)->getCsvAsStream(path);
    string[][]|error consumed = from string[] row in result
        select row;
    if consumed is ftp:ContentBindingError {
...
function testPutText() returns error? {
    ...
    check (<ftp:Client>clientEp)->putText(path, txt);
    string got = check (<ftp:Client>clientEp)->getText(path);
...
public function testPutCsvStringAndReadAllAndStream() returns error? {
    ...
    ftp:Error? resp = (<ftp:Client>clientEp)->putCsv(csvPath, csvData);
    if resp is ftp:Error {
        test:assertFail(msg = "Error while putting CSV string: " + resp.message());
    } else {
        log:printInfo("Executed `put` operation for CSV string");
    }

    string[][] gotAll = check (<ftp:Client>clientEp)->getCsv(csvPath);
    test:assertEquals(gotAll, csvData.slice(1), msg = "CSV text mismatch when reading all at once");

    stream<string[], error?> str = check (<ftp:Client>clientEp)->getCsvAsStream(csvPath);
    string[][] actual = [];
...
public function testPutCsvFromRecordsAndReadAllAndStream() returns error? {
    ...
    ftp:Error? resp = (<ftp:Client>clientEp)->putCsv(csvPath, records);
    if resp is ftp:Error {
        test:assertFail(msg = "Error while putting CSV string: " + resp.message());
    } else {
        log:printInfo("Executed `put` operation for CSV string");
    ...
    record {|string name; int age;|}[] gotAll = check (<ftp:Client>clientEp)->getCsv(csvPath);
    test:assertEquals(gotAll, records, msg = "CSV text mismatch when reading all at once");

    stream<record {|string name; int age;|}, error?> str = check (<ftp:Client>clientEp)->getCsvAsStream(csvPath);
...
public function testStreamClose() returns error? {
    ...
    stream<string[], error?> str1 = check (<ftp:Client>clientEp)->getCsvAsStream(csvPath);
    check str1.close();
...
function testFtpUserDirIsRootTrue() returns error? {
    stream<byte[] & readonly, io:Error?>|ftp:Error res = (<ftp:Client>ftpUserHomeRootClientEp)->get("test1.txt");
    if res is ftp:Error {
        test:assertFail("FTP get failed with userDirIsRoot=true: " + res.message());
    }
    
    stream<byte[] & readonly, io:Error?> str = res;
    test:assertTrue(check ftp_test_commons:matchStreamContent(str, "File content"), "Expected file content not found when userDirIsRoot=true");
...
public function testPutRelativePath_userDirIsRootTrue() returns error? {
    ftp:Error? putRes = (<ftp:Client>ftpUserHomeRootClientEp)->put(relativePath, "hello-jailed-rel");
    if putRes is ftp:Error {
        test:assertFail("PUT(relative, no slash) failed on userDirIsRoot=true: " + putRes.message());
    }

    stream<byte[] & readonly, io:Error?>|ftp:Error getRes = (<ftp:Client>ftpUserHomeRootClientEp)->get(relativePath);
    if getRes is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(getRes, "hello-jailed-rel"),
    ...
    ftp:Error? delRes = (<ftp:Client>ftpUserHomeRootClientEp)->delete(relativePath);
    if delRes is ftp:Error {
...
public function testPutRelativePathWithSlash_userDirIsRootTrue() returns error? {
    ftp:Error? putRes = (<ftp:Client>ftpUserHomeRootClientEp)->put(relativePathWithSlash, "hello-jailed-rel-with-slash");
    if putRes is ftp:Error {
        test:assertFail("PUT(relative, with slash) failed on userDirIsRoot=true: " + putRes.message());
    }

    stream<byte[] & readonly, io:Error?>|ftp:Error getRes = (<ftp:Client>ftpUserHomeRootClientEp)->get(relativePathWithSlash);
    if getRes is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(getRes, "hello-jailed-rel-with-slash"),
            msg = "Unexpected content from GET(relative) after PUT on userDirIsRoot=true");
        io:Error? closeErr = getRes.close();
        if closeErr is io:Error {
            test:assertFail("Error closing relative GET stream: " + closeErr.message());
    ...
    ftp:Error? delRes = (<ftp:Client>ftpUserHomeRootClientEp)->delete(relativePathWithSlash);
    if delRes is ftp:Error {
...
public function testPutAbsoluteDoubleSlash_userDirIsRootFalse() returns error? {
    ftp:Error? putRes = (<ftp:Client>clientEp)->put(absPath, "hello-abs-double-slash");
    if putRes is ftp:Error { test:assertFail("PUT(//absolute) failed: " + putRes.message()); }
    stream<byte[] & readonly, io:Error?>|ftp:Error getRes = (<ftp:Client>clientEp)->get(absPath);
    if getRes is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(getRes, "hello-abs-double-slash"));
        check getRes.close();
    } else { test:assertFail("GET(//absolute) failed: " + getRes.message()); }
    check (<ftp:Client>clientEp)->delete(absPath);
}
...
public function testSftpUserDirIsRootTrue_RelativePutGet() returns error? {
    ...
    ftp:Error? putRes = (<ftp:Client>sftpClientUserDirRootEp)->put("sftp-rel.txt", bStream);
    if putRes is ftp:Error { test:assertFail("SFTP relative PUT failed: " + putRes.message()); }
    stream<byte[] & readonly, io:Error?>|ftp:Error getRes = (<ftp:Client>sftpClientUserDirRootEp)->get("sftp-rel.txt");
    if getRes is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(getRes, "hello-sftp-rel"));
        check getRes.close();
    } else { test:assertFail("SFTP relative GET failed: " + getRes.message()); }
    check (<ftp:Client>sftpClientUserDirRootEp)->delete("sftp-rel.txt");
}
...
public function testPutLargeFileContent() returns error? {
    ...
    if str is stream<byte[] & readonly, io:Error?> {
        ...
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, expectedString),
...
function testPutBytesAsStreamNew() returns error? {
    ...
    ftp:Error? resp = (<ftp:Client>clientEp)->putBytesAsStream(path, s);
    if resp is ftp:Error {
...
function testPutCsvAsStreamWithStringRows() returns error? {
    ...
    ftp:Error? resp = (<ftp:Client>clientEp)->putCsvAsStream(csvPath, s);
    if resp is ftp:Error {
...
function testPutCsvAsStreamWithRecords() returns error? {
    ...
    Rec[] gotAll = check (<ftp:Client>clientEp)->getCsv(csvPath);
    test:assertEquals(gotAll, records, msg = "CSV record stream mismatch (get all)");

    stream<Rec, error?> str = check (<ftp:Client>clientEp)->getCsvAsStream(csvPath);
    Rec[] actual = [];
...
function testGetBytesAndTextNonExistent() returns error? {
    ...
    var r1 = (<ftp:Client>clientEp)->getBytes(missing);
    test:assertTrue(r1 is ftp:Error, msg = "getBytes should error for missing file");

    var r2 = (<ftp:Client>clientEp)->getText(missing);
    test:assertTrue(r2 is ftp:Error, msg = "getText should error for missing file");
}
...
function testJsonTypedBinding_strict_and_lax() returns error? {
    ...
    PersonStrict|ftp:Error strictRes = (<ftp:Client>clientEp)->getJson(path);
    test:assertTrue(strictRes is ftp:Error, msg = "Strict JSON binding should fail when required field absent");

    PersonLax laxVal = check (<ftp:Client>clientEpLaxDataBinding)->getJson(path);
...
function testXmlTypedBinding_strict_and_lax() returns error? {
    ...
    var strictRes = (<ftp:Client>clientEp)->getXml(path, XPersonStrict);
    test:assertTrue(strictRes is ftp:Error, msg = "Strict XML binding should fail when required field absent");

    XPersonLax laxVal = check (<ftp:Client>clientEpLaxDataBinding)->getXml(path, XPersonLax);
    test:assertEquals(laxVal.name, "Alice");
}
...
function testCsvTypedBinding_strict_and_lax() returns error? {
    ...
    check (<ftp:Client>clientEp)->putCsv(csvPath, csvData, ftp:OVERWRITE);

    // Strict binding should fail when required field is absent
    CsvPersonStrict[]|ftp:Error strictRes = (<ftp:Client>clientEp)->getCsv(csvPath);
    test:assertTrue(strictRes is ftp:Error, msg = "Strict CSV binding should fail when required field absent");

    // Lax binding should succeed with nil for missing field
    CsvPersonLax[] laxVal = check (<ftp:Client>clientEpLaxDataBinding)->getCsv(csvPath);
...
function testCsvStreamTypedBinding_strict_and_lax() returns error? {
    ...
    stream<CsvPersonStrict, error?>|ftp:Error strictStreamRes = (<ftp:Client>clientEp)->getCsvAsStream(csvPath);
    if strictStreamRes is stream<CsvPersonStrict, error?> {
        ...
        test:assertTrue(strictStreamRes is ftp:ContentBindingError,
            msg = "Strict CSV stream should return ContentBindingError at creation");
    }

    // Lax streaming should succeed
    stream<CsvPersonLax, error?> laxStream = check (<ftp:Client>clientEpLaxDataBinding)->getCsvAsStream(csvPath);
...
function testPutTextWithAppendOption() returns error? {
    ...
    check (<ftp:Client>clientEp)->putText(path, "Hello", ftp:OVERWRITE);
    check (<ftp:Client>clientEp)->putText(path, " + world", ftp:APPEND);
    string got = check (<ftp:Client>clientEp)->getText(path);
...
function testPutCsvWithAppendOption() returns error? {
    ...
    ftp:Error? r1 = (<ftp:Client>clientEp)->putCsv(path, first, ftp:OVERWRITE);
    if r1 is ftp:Error {
    ...
    ftp:Error? r2 = (<ftp:Client>clientEp)->putCsv(path, more, ftp:APPEND);
    if r2 is ftp:Error {
        test:assertFail("putCsv append failed: " + r2.message());
    }

    string[][] gotAll = check (<ftp:Client>clientEp)->getCsv(path);
...
public function testPutTextContent() returns error? {
    ...
    ftp:Error? response = (<ftp:Client>clientEp)->put(filePath, textToPut);
    if response is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(filePath);
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, "Sample text content"),
...
public function testPutJsonContent() returns error? {
    ...
    ftp:Error? response = (<ftp:Client>clientEp)->put(filePath, jsonToPut);
    if response is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(filePath);
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, "{\"name\":\"Anne\", \"age\":20}"),
...
public function testPutXMLContent() returns error? {
    ...
    ftp:Error? response = (<ftp:Client>clientEp)->put(filePath, xmlToPut);
    if response is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(filePath);
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertTrue(check ftp_test_commons:matchStreamContent(str, "<note><heading>Memo</heading><body>Memo content</body></note>"),
...
public function testIsDirectory() {
    boolean|ftp:Error response1 = (<ftp:Client>clientEp)->isDirectory("/home/in");
    ...
    boolean|ftp:Error response2 = (<ftp:Client>clientEp)->isDirectory(filePath);
...
public function testCreateDirectory() {
    ftp:Error? response1 = (<ftp:Client>clientEp)->mkdir("/home/in/out");
    if response1 is ftp:Error {
        test:assertFail(msg = "Error while creating a directory" + response1.message());
    } else {
        log:printInfo("Executed `mkdir` operation");
    }

    boolean|ftp:Error response2 = (<ftp:Client>clientEp)->isDirectory("/home/in/out");
...
public function testRenameDirectory() {
    ...
    ftp:Error? response1 = (<ftp:Client>clientEp)->rename(existingName, newName);
    if response1 is ftp:Error {
    ...
    boolean|ftp:Error response2 = (<ftp:Client>clientEp)->isDirectory(existingName);
    log:printInfo("Executed `isDirectory` operation on original directory after renaming a directory");
    if response2 is ftp:Error {
    ...
    boolean|ftp:Error response3 = (<ftp:Client>clientEp)->isDirectory(newName);
...
public function testMoveFile() {
    ...
    ftp:Error? putResponse = (<ftp:Client>clientEp)->put(sourcePath, testContent);
    if putResponse is ftp:Error {
        test:assertFail(msg = "Error while creating test file for `move` operation: " + putResponse.message());
    }
    
    // Now move the file
    ftp:Error? moveResponse = (<ftp:Client>clientEp)->move(sourcePath, destinationPath);
    if moveResponse is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error sourceCheck = (<ftp:Client>clientEp)->get(sourcePath);
    if sourceCheck is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error destCheck = (<ftp:Client>clientEp)->get(destinationPath);
    ...
    // Cleanup
    ftp:Error? deleteResponse = (<ftp:Client>clientEp)->delete(destinationPath);
...
public function testCopyFile() {
    ...
    boolean|ftp:Error sourceExists = (<ftp:Client>clientEp)->exists(sourcePath);
    ...
    // Copy the file
    ftp:Error? copyResponse = (<ftp:Client>clientEp)->copy(sourcePath, destinationPath);
    if copyResponse is ftp:Error {
    ...
    boolean|ftp:Error sourceStillExists = (<ftp:Client>clientEp)->exists(sourcePath);
    ...
    boolean|ftp:Error destExists = (<ftp:Client>clientEp)->exists(destinationPath);
...
public function testExistsFile() {
    ...
    boolean|ftp:Error existsResult = (<ftp:Client>clientEp)->exists(existingPath);
    ...
    boolean|ftp:Error notExistsResult = (<ftp:Client>clientEp)->exists(nonExistingPath);
...
public function testGetFileSize() {
    int|ftp:Error response = (<ftp:Client>clientEp)->size(filePath);
...
public function testListFiles() {
    ...
    ftp:FileInfo[]|ftp:Error response = (<ftp:Client>clientEp)->list("/home/in");
    if response is ftp:FileInfo[] {
        log:printInfo("List of files/directories: ");
        map<ftp:FileInfo> fileInfoMap = {};
        ...
        foreach string expectedPath in expectedSizes.keys() {
            ftp:FileInfo? expectedInfo = fileInfoMap[expectedPath];
            test:assertTrue(expectedInfo is ftp:FileInfo,
                msg = "Expected path not found during the `list` operation: " + expectedPath);
            if expectedInfo is ftp:FileInfo {
...
public function testDeleteFile() returns error? {
    ftp:Error? response = (<ftp:Client>clientEp)->delete(filePath);
    if response is ftp:Error {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error str = (<ftp:Client>clientEp)->get(filePath);
    if str is stream<byte[] & readonly, io:Error?> {
        test:assertFalse(check ftp_test_commons:matchStreamContent(str, "<note><heading>Memo</heading><body>Memo content</body></note>"),
...
function testGenericRmdir(string path) returns error? {
    boolean|ftp:Error response0 = (<ftp:Client>clientEp)->isDirectory(path);
    log:printInfo("Executed `isDirectory` operation before deleting a directory " + path);
    int retryCount = 0;
    while (response0 is ftp:Error || !response0) && retryCount < 10 {
        log:printInfo("Executed `isDirectory` operation before deleting a directory " + path);
        runtime:sleep(1);
        response0 = (<ftp:Client>clientEp)->isDirectory(path);
        retryCount += 1;
    }
    ...
    if retryCount >= 10 {
        ...
        ftp:Error? response1 = (<ftp:Client>clientEp)->rmdir(path);
        if response1 is ftp:Error {
    ...
    boolean|ftp:Error response2 = (<ftp:Client>clientEp)->isDirectory(path);
    log:printInfo("Executed `isDirectory` operation after deleting a directory " + path);

    int i = 0;
    while response2 is boolean && i < 10 {
        runtime:sleep(1);
        response2 = (<ftp:Client>clientEp)->isDirectory(path);
        log:printInfo("Executed `isDirectory` operation after deleting a directory " + path);
        i += 1;
    }
    if response2 is ftp:Error {
...
public function testClientClose() returns error? {
    ...
    boolean|ftp:Error existsResult = ftpClient->exists(filePath);
    test:assertTrue(existsResult is ftp:Error);
    if existsResult is ftp:Error {
    ...
    ftp:Error? putResult = ftpClient->put(newFilePath, byteStream);
    test:assertTrue(putResult is ftp:Error);
...
public function testCloseThenPutApis() returns error? {
    ...
    ftp:Error? putBytes = ftpClient->putBytes("/home/in/after-close-putBytes.txt", "abc".toBytes());
    test:assertTrue(putBytes is ftp:Error);
    if putBytes is ftp:Error {
        test:assertEquals(putBytes.message(), closeErrorMessage);
    }

    ftp:Error? putText = ftpClient->putText("/home/in/after-close-putText.txt", "abc");
    test:assertTrue(putText is ftp:Error);
    if putText is ftp:Error {
        test:assertEquals(putText.message(), closeErrorMessage);
    }

    json jsonPayload = {"a": 1};
    ftp:Error? putJson = ftpClient->putJson("/home/in/after-close-putJson.json", jsonPayload);
    test:assertTrue(putJson is ftp:Error);
    if putJson is ftp:Error {
        test:assertEquals(putJson.message(), closeErrorMessage);
    }

    ftp:Error? putXml = ftpClient->putXml("/home/in/after-close-putXml.xml", xml `<a>1</a>`);
    test:assertTrue(putXml is ftp:Error);
    if putXml is ftp:Error {
        test:assertEquals(putXml.message(), closeErrorMessage);
    }

    ftp:Error? putCsv = ftpClient->putCsv("/home/in/after-close-putCsv.csv", [["id", "name"], ["1", "A"]]);
    test:assertTrue(putCsv is ftp:Error);
    if putCsv is ftp:Error {
    ...
    ftp:Error? putBytesAsStream = ftpClient->putBytesAsStream("/home/in/after-close-putBytesAsStream.txt", bytesStream);
    test:assertTrue(putBytesAsStream is ftp:Error);
    if putBytesAsStream is ftp:Error {
    ...
    ftp:Error? putCsvAsStream = ftpClient->putCsvAsStream("/home/in/after-close-csv-stream.csv", csvStream);
    test:assertTrue(putCsvAsStream is ftp:Error);
    if putCsvAsStream is ftp:Error {
...
public function testCloseThenGetApis() returns error? {
    ...
    stream<byte[] & readonly, io:Error?>|ftp:Error getResult = ftpClient->get("/home/in/test1.txt");
    test:assertTrue(getResult is ftp:Error);
    if getResult is ftp:Error {
        test:assertEquals(getResult.message(), closeErrorMessage);
    }

    byte[]|ftp:Error getBytesResult = ftpClient->getBytes("/home/in/test1.txt");
    test:assertTrue(getBytesResult is ftp:Error);
    if getBytesResult is ftp:Error {
        test:assertEquals(getBytesResult.message(), closeErrorMessage);
    }

    string|ftp:Error getTextResult = ftpClient->getText("/home/in/test1.txt");
    test:assertTrue(getTextResult is ftp:Error);
    if getTextResult is ftp:Error {
        test:assertEquals(getTextResult.message(), closeErrorMessage);
    }

    json|ftp:Error getJsonResult = ftpClient->getJson("/home/in/test1.txt");
    test:assertTrue(getJsonResult is ftp:Error);
    if getJsonResult is ftp:Error {
        test:assertEquals(getJsonResult.message(), closeErrorMessage);
    }

    xml|ftp:Error getXmlResult = ftpClient->getXml("/home/in/test1.txt");
    test:assertTrue(getXmlResult is ftp:Error);
    if getXmlResult is ftp:Error {
        test:assertEquals(getXmlResult.message(), closeErrorMessage);
    }

    string[][]|ftp:Error getCsvResult = ftpClient->getCsv("/home/in/test1.txt");
    test:assertTrue(getCsvResult is ftp:Error);
    if getCsvResult is ftp:Error {
        test:assertEquals(getCsvResult.message(), closeErrorMessage);
    }

    stream<byte[], error?>|ftp:Error getBytesAsStreamResult = ftpClient->getBytesAsStream("/home/in/test1.txt");
    test:assertTrue(getBytesAsStreamResult is ftp:Error);
    if getBytesAsStreamResult is ftp:Error {
        test:assertEquals(getBytesAsStreamResult.message(), closeErrorMessage);
    }

    stream<string[], error?>|ftp:Error getCsvAsStreamResult = ftpClient->getCsvAsStream("/home/in/test1.txt");
    test:assertTrue(getCsvAsStreamResult is ftp:Error);
    if getCsvAsStreamResult is ftp:Error {
...
public function testCloseThenOtherApis() returns error? {
    ...
    ftp:Error? moveResult = ftpClient->move("/home/in/test1.txt", "/home/in/after-close-move.txt");
    test:assertTrue(moveResult is ftp:Error);
    if moveResult is ftp:Error {
        test:assertEquals(moveResult.message(), closeErrorMessage);
    }

    ftp:Error? copyResult = ftpClient->copy("/home/in/test1.txt", "/home/in/after-close-copy.txt");
    test:assertTrue(copyResult is ftp:Error);
    if copyResult is ftp:Error {
        test:assertEquals(copyResult.message(), closeErrorMessage);
    }

    ftp:Error? appendResult = ftpClient->append("/home/in/after-close-append.txt", "abc");
    test:assertTrue(appendResult is ftp:Error);
    if appendResult is ftp:Error {
        test:assertEquals(appendResult.message(), closeErrorMessage);
    }

    ftp:Error? mkdirResult = ftpClient->mkdir("/home/in/after-close-dir");
    test:assertTrue(mkdirResult is ftp:Error);
    if mkdirResult is ftp:Error {
        test:assertEquals(mkdirResult.message(), closeErrorMessage);
    }

    ftp:Error? rmdirResult = ftpClient->rmdir("/home/in/after-close-dir");
    test:assertTrue(rmdirResult is ftp:Error);
    if rmdirResult is ftp:Error {
        test:assertEquals(rmdirResult.message(), closeErrorMessage);
    }

    ftp:Error? renameResult = ftpClient->rename("/home/in/test1.txt", "/home/in/after-close-rename.txt");
    test:assertTrue(renameResult is ftp:Error);
    if renameResult is ftp:Error {
        test:assertEquals(renameResult.message(), closeErrorMessage);
    }

    boolean|ftp:Error isDirResult = ftpClient->isDirectory("/home/in");
    test:assertTrue(isDirResult is ftp:Error);
    if isDirResult is ftp:Error {
        test:assertEquals(isDirResult.message(), closeErrorMessage);
    }

    int|ftp:Error sizeResult = ftpClient->size("/home/in/test1.txt");
    test:assertTrue(sizeResult is ftp:Error);
    if sizeResult is ftp:Error {
        test:assertEquals(sizeResult.message(), closeErrorMessage);
    }

    ftp:FileInfo[]|ftp:Error listResult = ftpClient->list("/home/in");
    test:assertTrue(listResult is ftp:Error);
    if listResult is ftp:Error {
        test:assertEquals(listResult.message(), closeErrorMessage);
    }

    ftp:Error? deleteResult = ftpClient->delete("/home/in/test1.txt");
    test:assertTrue(deleteResult is ftp:Error);
