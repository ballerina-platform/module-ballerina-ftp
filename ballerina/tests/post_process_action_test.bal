...
public function testAfterProcessMoveWithPreserveSubDirsTrue() returns error? {
    ...
    Service movePreserveService = service object {
        @FunctionConfig {
    ...
    // Create listener for subdirectory
    Listener preserveListener = check new ({
        protocol: FTP,
        host: "127.0.0.1",
        auth: {credentials: {username: "wso2", password: "wso2123"}},
        port: 21212,
    ...
    check (<Client>clientEp)->putJson(testFilePath, <json>{"name": "test", "preserveSubDirs": true});
    runtime:sleep(15);
    ...
    // Cleanup
    check (<Client>clientEp)->delete(archiveFilePath);
    check (<Client>clientEp)->rmdir(POST_PROCESS_ARCHIVE_DIR + "/preserve-subdir");
    check (<Client>clientEp)->rmdir(subDir);
}
...
function cleanupPostProcessTestDirectories() returns error? {
    ...
    if testDirExists {
        ...
        boolean preserveSubDirExists = check (<Client>clientEp)->exists(POST_PROCESS_TEST_DIR + "/preserve-subdir");
        if preserveSubDirExists {
            FileInfo[] preserveFiles = check (<Client>clientEp)->list(POST_PROCESS_TEST_DIR + "/preserve-subdir");
            foreach FileInfo file in preserveFiles {
                if file.isFile {
                    check (<Client>clientEp)->delete(file.path);
                }
            }
            check (<Client>clientEp)->rmdir(POST_PROCESS_TEST_DIR + "/preserve-subdir");
        }
    }
    ...
    if archiveDirExists {
        ...
        boolean preserveArchiveDirExists = check (<Client>clientEp)->exists(POST_PROCESS_ARCHIVE_DIR +
            "/preserve-subdir");
        if preserveArchiveDirExists {
            FileInfo[] preserveArchiveFiles = check (<Client>clientEp)->list(POST_PROCESS_ARCHIVE_DIR +
                "/preserve-subdir");
            foreach FileInfo file in preserveArchiveFiles {
                if file.isFile {
                    check (<Client>clientEp)->delete(file.path);
                }
            }
