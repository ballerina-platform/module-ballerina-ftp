...
public function testBinaryModeIntegrity() returns error? {
    ClientConfiguration binaryConfig = {
        ...
        port: 21212,
        auth: {credentials: {username: "wso2", password: "wso2123"}},
        ftpFileType: BINARY  // Explicit BINARY mode
    };
    ...
    if ftpClient is Error {
        ...
        byte[] originalContent = check io:fileReadBytes("tests/resources/vfs_test_data/binary_test.bin");
        ...
        stream<io:Block, io:Error?> bStream = check io:fileReadBlocksAsStream(
            "tests/resources/vfs_test_data/binary_test.bin", 1024);
...
public function testAsciiModeCrlfTranslation() returns error? {
    ClientConfiguration asciiConfig = {
        ...
        port: 21212,
        auth: {credentials: {username: "wso2", password: "wso2123"}},
        ftpFileType: ASCII  // Explicit ASCII mode
    };
    ...
    if ftpClient is Error {
        ...
        stream<io:Block, io:Error?> textStream = check io:fileReadBlocksAsStream(
            "tests/resources/vfs_test_data/text_lf.txt", 1024);
...
public function testDefaultFileTypeIsBinary() returns error? {
    ClientConfiguration defaultConfig = {
        protocol: FTP,
        host: "127.0.0.1",
        port: 21212,
        auth: {credentials: {username: "wso2", password: "wso2123"}}
        // ftpFileType not specified - should default to BINARY
    };

    test:assertEquals(defaultConfig.ftpFileType, BINARY,
